...
// file_transfer.c -
//
// Created by wwq on 2021/09/13
// Last Modified: 2021/09/13 08:29:41
//
//=====================================================================
#include <arpa/inet.h>
#include <limits.h>
...
int connectForRecv(const char *ipaddr, short port) {
  ...
  if (sockfd == SOCK_ERROR)
    ERR_EXIT("socket erro");

  struct sockaddr_in serveraddr;
  memset(&serveraddr, 0, sizeof serveraddr);
  serveraddr.sin_family = AF_INET;
  serveraddr.sin_addr.s_addr =
      inet_addr(ipaddr == NULL ? DEFAULT_ADDR : ipaddr);
...
void closeConnect(int sockefd) {
  if (close(sockefd) == SOCK_ERROR)
...
void usage(void) {
  printf("\nft v%s (Usage)\n"
         ...
         "recv : ft -r [-l <filepath>] [-h <ipaddr>] [-p <port>] [-P "
         "<password>]\n\n",
         FT_VERSION);
  exit(EXIT_FAILURE);
}
